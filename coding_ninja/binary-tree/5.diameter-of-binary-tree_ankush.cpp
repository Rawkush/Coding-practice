...
BinaryTreeNode<int>* buildTree(int *postorder, int postLength, int *inorder, int inLength) {
  ...
  pair *getDiameter(TreeNode* root){
    if(root==NULL) return new pair();
    pair* left =  getDiameter(root->left);
    pair* right = getDiameter(root->right);

    pair* rootPair = new pair();
    // if(left->height + right->height > max(left->height,right->height)){
    //     pair->d
    // }
    rootPair->dia =  max(left->height + right->height,
                         max(left->dia, right->dia) );
    rootPair->height =  1+ max(left->height, right->height);
    return  rootPair;
  }
